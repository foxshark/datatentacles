...
class InstagramClient
{
	...
	// NEW STUFF
	storePostOwner(owner) {
		var self = this;
		connection.query(`
			INSERT INTO ig_users_short
				(user_id, username, posts_count, followers_count, is_private, is_verified)
			VALUES
				(?)
			ON DUPLICATE KEY UPDATE 
				posts_count = VALUES(posts_count), 
				followers_count = VALUES(followers_count)
			`, [self.formatOwnerToUser(owner)], function (error) {
				if (error) {
					throw error;
				}
			});	
	...
	wrirteUser(user) {
		...
		connection.query(`
			INSERT INTO ig_users_short
				(user_id, username, full_json, posts_count, followers_count, is_private, is_verified)
			VALUES
				(?)
			ON DUPLICATE KEY UPDATE 
				full_json = VALUES(full_json), # make sure to if-not-null?
				posts_count = VALUES(posts_count), 
				followers_count = VALUES(followers_count)
			`, [user], function (error) {
				if (error) {
	...
	storeUser(user) {
		this.wrirteUser(this.formatUser(user));
	}
	...
	formatOwnerToUser(owner) {
		/*
		// Owner Format
	  	id: '6903183059',
	    username: 'fortheloveofgrain',
	    profilePicture: 'https://instagram.fden3-1.fna.fbcdn.net/v/t51.2885-19/s150x150/80839473_749704052189397_1302230635288461312_n.jpg?_nc_ht=instagram.fden3-1.fna.fbcdn.net&_nc_ohc=XPC58rKPjx8AX9dtU-W&tp=1&oh=b974970f145cc739b8073678b6f82b57&oe=5FF5F07E',
	    full_name: 'Dustin Cogsdell',
	    postsCount: { count: 167 },
	    followersCount: { count: 679 },
	    isPrivate: false,
	    isVerified: false
	    */
		return [
		    ...
		    // owner.full_name,
		    owner.postsCount.count,
		    owner.followersCount.count,
		    owner.isPrivate,
		    owner.isVerified
		];
	}
...
//<script type="application/ld+json">
	...
	fetchFeedGraph(feedID, feedName)
	{
		...
		var self = this;
 		var headerObj = { 'User-Agent': 'tentacular' };
		axios.get(feedURL, {headers:headerObj} )
		  .then(function (response) {
		    // handle success
			...
			if(typeof(response.data.graphql) == "undefined")
			{
